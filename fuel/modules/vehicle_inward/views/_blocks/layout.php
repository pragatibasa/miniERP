...
$(function() {
  $( "#date" ).picker({
    onSelect: function(datetext, inst) {
      $.ajax({
          type: "POST",
          url: "<?php echo fuel_url('vehicle_inward/getInwardVehiclesWithDate');?>",
          data: "date="+datetext,
          dataType: "json"
          }).done(function( msg ) {
            if(msg.length) {
          ...
          if(msg.length == 0) {
            ...
            $('#contentprocess').html(loading1);
                ...
                for (var i = 0; i < msg.length; i++) {
                  ...
                  thisdata["Inward Date"] = item.dReceivedDate;
                  ...
                  thisdata["Weight in kgs"] = item.fQuantity;
...
function saveToExcel() {
    ...
    tab_text = tab_text + '<table><tr><td style="font-size:60px; font-style:italic; font-family:fantasy;" colspan="7" align="center"><h1>Inward Vehicle Report</h1></td></tr>';

    tab_text = tab_text + '<tr></tr><tr><td><b>Inward Vehicle Date: </b></td><td><b>'+$('#date').val()+'</b></td></tr>' +
    ...
    if (msie > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./)) {
        ...
        $('#export').attr('download', 'vehicle_inward_report_'+$('#date').val()+'.xls');
    }
}
</script>
