...
//			}
	...
	function recentcust()
	{
		$sql = "select cust_id, name, phone, city from fuel_customers order by customer_id desc LIMIT 5";
	...
	function recentwip()
	{
		$sql = "select aspen_tblinwardentry.vIRnumber as coilnumber,aspen_tblpartydetails.nPartyName as partyname,aspen_tblcuttinginstruction.dDate as cuttingdate,aspen_tblrecoiling.dStartDate as recoilingdate,aspen_tblslittinginstruction.dDate as slittingdate From aspen_tblinwardentry LEFT JOIN aspen_tblmatdescription  ON aspen_tblmatdescription.nMatId=aspen_tblinwardentry.nMatId LEFT JOIN aspen_tblcuttinginstruction  ON aspen_tblcuttinginstruction.vIRnumber=aspen_tblinwardentry.vIRnumber LEFT JOIN aspen_tblrecoiling  ON aspen_tblrecoiling .vIRnumber=aspen_tblinwardentry.vIRnumber LEFT JOIN aspen_tblpartydetails ON aspen_tblpartydetails .nPartyId=aspen_tblinwardentry.nPartyId 
		LEFT JOIN aspen_tblslittinginstruction ON aspen_tblslittinginstruction .vIRnumber=aspen_tblinwardentry.vIRnumber
		where aspen_tblinwardentry.vStatus = 'Work In Progress' or aspen_tblslittinginstruction.vStatus='WIP-Slitting' or aspen_tblrecoiling.vStatus='WIP-Recoiling' or aspen_tblcuttinginstruction.vStatus='WIP-Cutting' Group by aspen_tblinwardentry.vIRnumber desc LIMIT 3";
		$query = $this->db->query($sql);
		$arr='';
		if ($query->num_rows() > 0) {
		 	foreach ($query->result() as $row)
	...
	function recentvend()
	{
		$sql = "select name, phone, city, state_id from fuel_suppliers order by supplier_id desc LIMIT 5";
		$query = $this->db->query($sql);
		$arr='';
		if ($query->num_rows() > 0) {
		 	foreach ($query->result() as $row)
		...
		//$rvend = json_encode($arr);
		return $arr;
	}
	
	function recentbill()
	{
		$sql = "select id, item_count, total_amount, main_name, main_city from fuel_journal_main where main_type = 'ib' order by id desc LIMIT 5";
		$query = $this->db->query($sql);
		$arr='';
		if ($query->num_rows() > 0) {
		 	foreach ($query->result() as $row)
		...
		//$rbill = json_encode($arr);
		return $arr;
	}
	
	function recentinv()
	{
		$sql = "select item_sku, item_name, quantity_on_hand, item_cost from fuel_inventory_lists order by item_id desc LIMIT 5";
		$query = $this->db->query($sql);
		$arr='';
		if ($query->num_rows() > 0) {
		 	foreach ($query->result() as $row)
		...
		//$radj = json_encode($arr);
		return $arr;
	}
	
	function recentbilldes()
	{
		$sql = "select aspen_tblbilldetails.nBillNo as Billno,aspen_tblbilldetails.vIRnumber as CoilNumber,aspen_tblbilldetails.dBillDate as BillDate,aspen_tblbilldetails.fWeightAmount as WeightAmount,aspen_tblbilldetails.fGrantTotal as GrandTotal,aspen_tblbilldetails.vOutLorryNo as LorryNo,aspen_tblbilldetails.vBillType as BillType,aspen_tblbilldetails.nPartyId from aspen_tblbilldetails LIMIT 5";
		$query = $this->db->query($sql);
		$arr='';
		if ($query->num_rows() > 0) {
		 	foreach ($query->result() as $row)
	...
	function recenttax()
	{
		$sql = "select nTaxTypeId as taxid,vTypeOfTax as type,nPercentage as percentage from aspen_tbltaxdetails ";
		$query = $this->db->query($sql);
		$arr='';
		if ($query->num_rows() > 0) {
		 	foreach ($query->result() as $row)
